...
const CITY = {
    ...
    ROAD_WIDTH: 16,
    BLOCK_SIZE: 90,          // taille des îlots
    BUILDING_MIN: 24,
    BUILDING_MAX: 90,
    LAMP_EVERY: 60,
    LAMP_HEIGHT: 22,
...
function createRoadGridTexture(size, blockSize, roadWidth) {
    ...
    for (let x = 0; x <= cvs.width; x += step) {
        ctx.fillRect(x - w * 0.5, 0, w, cvs.height);
    }
    for (let y = 0; y <= cvs.height; y += step) {
        ctx.fillRect(0, y - w * 0.5, cvs.width, w);
    }
...
function makeWindowTexture(w=128, h=256, hue=0.6) {
    ...
    for (let i=0;i<cols;i++){
        for (let j=0;j<rows;j++){
            ...
            const s = on ? 70 : 10;
            const color = hsl(hue + (Math.random()*0.06-0.03), s, l);
            ctx.fillStyle = color;
...
function createBuilding(x, z, w, d, h) {
    ...
    const tex = makeWindowTexture(128, 256, 0.58 + Math.random()*0.05);
    ...
    const roofGeo = new THREE.BoxGeometry(w*1.02, 2, d*1.02);
    const roof = new THREE.Mesh(roofGeo, new THREE.MeshStandardMaterial({ color: 0x1c1f24, roughness: 0.8, metalness: 0.2 }));
...
function createLamp(x, z, height=CITY.LAMP_HEIGHT) {
    ...
    const light = new THREE.PointLight(0xffe9aa, 1.2, 80, 2);
...
function populateCity() {
    // anneau d’immeubles autour du lac
    const tryCount = 500;
    const min = CITY.RING_INNER, max = CITY.RING_OUTER;
    ...
    for (let i=0;i<tryCount;i++){
        ...
        const h = THREE.MathUtils.randInt(CITY.BUILDING_MIN, CITY.BUILDING_MAX) * (Math.random()<0.2 ? 1.8 : 1);

        createBuilding(x, z, w, d, h);
    }

    // lampadaires le long de l’anneau intérieur
    const perimeter = 2*Math.PI*CITY.RING_INNER;
    const lampCount = Math.floor(perimeter / CITY.LAMP_EVERY);
...
function generatePlants() {
    ...
    const count = 40; // Nombre de plantes
